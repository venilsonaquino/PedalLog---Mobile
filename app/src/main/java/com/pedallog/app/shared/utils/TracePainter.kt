...
class TracePainter(
    ...
    fun drawFrame(points: List<PedalPoint>, index: Int): Bitmap {
        ...
        val path = Path().apply {
            moveTo(mapper.getX(points[0].coordinate.longitude), mapper.getY(points[0].coordinate.latitude))
            for (j in 1..index) lineTo(mapper.getX(points[j].coordinate.longitude), mapper.getY(points[j].coordinate.latitude))
        }
        canvas.drawPath(path, paintTrace)
        val cx = mapper.getX(points[index].coordinate.longitude)
        val cy = mapper.getY(points[index].coordinate.latitude)
